...
##
## Developed by:
## Chun Wong (email@chunwong.net)
## Revised by: Hiroki Sayama (sayama@binghamton.edu)
## Additional fixes/notes in header of original file.
##
## IMPORTANT: keep these two lines at the very beginning of your simulator code:
## import matplotlib
...
import pylab as PL  # or: import matplotlib.pyplot as PL
from tkinter import *              # updated by Sriniwas Pandey for Python 3
from tkinter import ttk            # updated by Sriniwas Pandey for Python 3
from tkinter.ttk import Notebook   # updated by Sriniwas Pandey for Python 3
...
class GUI:
    ...
    def __init__(self, title='PyCX Simulator', interval=0, stepSize=1, parameterSetters=[]):
        ...
        self.varEntries = {}
        self.statusStr = ""
        self._after_id = None  # updated by Sriniwas Pandey for Python 3 (safe Tk scheduling)
    ...
    def initGUI(self):
        ...
        self.notebook = Notebook(self.rootWindow)  # updated by Sriniwas Pandey for Python 3
        self.notebook.pack(side=TOP, padx=2, pady=2)

        # Create frames as children of the Notebook (prevents orphaned .!frameNN)
        self.frameRun = Frame(self.notebook)          # updated by Sriniwas Pandey for Python 3
        self.frameSettings = Frame(self.notebook)     # updated by Sriniwas Pandey for Python 3
        self.frameParameters = Frame(self.notebook)   # updated by Sriniwas Pandey for Python 3
        self.frameInformation = Frame(self.notebook)  # updated by Sriniwas Pandey for Python 3
        ...
        self.showHelp(self.stepDelay, "The visualization of each step is delayed by the given number of milliseconds.")  # updated by Sriniwas Pandey for Python 3 (text tweak)
        ...
        for variableSetter in self.parameterSetters:
            ...
            if variableSetter.__doc__ is not None and len(variableSetter.__doc__) > 0:
    ...
    # <<<< runEvent >>>>>
    # This event is invoked when "Run" button is clicked.
    def runEvent(self):
        self.running = not self.running
        if self.running:
            # schedule first step
            self._after_id = self.rootWindow.after(self.timeInterval, self.stepModel)  # updated by Sriniwas Pandey for Python 3 (safe scheduling)
            ...
            # paused
            if self._after_id is not None:  # updated by Sriniwas Pandey for Python 3
                try:
                    self.rootWindow.after_cancel(self._after_id)  # updated by Sriniwas Pandey for Python 3
                except Exception:
                    pass
                self._after_id = None  # updated by Sriniwas Pandey for Python 3
    ...
    def stepModel(self):
        # if window destroyed or no longer running, stop
        if not self.running or not self.rootWindow.winfo_exists():  # updated by Sriniwas Pandey for Python 3
            return
        self.modelStepFunc()
        self.currentStep += 1
        ...
        if (self.currentStep) % self.stepSize == 0:
            self.drawModel()
        delay = int(self.timeInterval * 1.0 / max(1, self.stepSize))  # updated by Sriniwas Pandey for Python 3
        self._after_id = self.rootWindow.after(delay, self.stepModel)  # updated by Sriniwas Pandey for Python 3
    ...
    def drawModel(self):
        PL.ion()  # interactive
        # Create a figure if needed (handles the case where it was closed)
        if self.modelFigure is None or not PL.fignum_exists(self.modelFigure.number):  # updated by Sriniwas Pandey for Python 3
            self.modelFigure = PL.figure()  # updated by Sriniwas Pandey for Python 3
        self.modelDrawFunc()
        # Be defensive about manager/window availability
        try:
            mgr = self.modelFigure.canvas.manager
            if hasattr(mgr, "window") and mgr.window.winfo_exists():  # updated by Sriniwas Pandey for Python 3
                mgr.window.update()
            else:
                self.modelFigure.canvas.draw_idle()  # updated by Sriniwas Pandey for Python 3
        except Exception:
            self.modelFigure.canvas.draw_idle()  # updated by Sriniwas Pandey for Python 3
        PL.show()
    ...
    def start(self, func=[]):
        if len(func) == 3:
            ...
            if (self.modelStepFunc.__doc__ is not None and len(self.modelStepFunc.__doc__) > 0):
                self.showHelp(self.buttonStep, self.modelStepFunc.__doc__.strip())
            if (self.modelInitFunc.__doc__ is not None and len(self.modelInitFunc.__doc__) > 0):
    ...
    def quitGUI(self):
        # stop scheduling and close cleanly
        self.running = False
        if self._after_id is not None:  # updated by Sriniwas Pandey for Python 3
            try:
                self.rootWindow.after_cancel(self._after_id)  # updated by Sriniwas Pandey for Python 3
